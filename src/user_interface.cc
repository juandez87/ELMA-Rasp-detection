...
void UserInterface::update() {
    ...
    // OUTPUT
    show_time(1,1,_detection.value()); 
    mvprintw(3,1,"start(s), quit(q)");
    for ( int i=0; i<_detection.laps().size(); i++ ) {
        mvprintw(5+i, 1, "Lap %d", _detection.laps().size()-i);
        show_time(5+i, 10, _detection.laps()[i]);
    }

    // NOTE: Since the Detection is running every 10 ms, we should sleep
